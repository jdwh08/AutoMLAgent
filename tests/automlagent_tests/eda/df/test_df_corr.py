...
### OWN MODULES
from automlagent.dataclass.df_info import ColumnInfo, DataFrameInfo
from automlagent.eda.df.df_corr import get_correlation, get_pearson_correlation
...
class TestDfCorr:
    def test_get_pearson_correlation_expected(self) -> None:
        ...
        result = get_pearson_correlation(df, df_info)
        ...
        # Off-diagonal: check that correlation is correct for perfectly correlated columns
    ...
    def test_get_correlation_expected(self) -> None:
        ...
        corr = get_correlation("target", "a", df_info)
    ...
    def test_get_correlation_missing_matrix(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as w:
            result = get_correlation("target", "a", df_info)
    ...
    def test_get_correlation_missing_column(self) -> None:
        ...
        df_info = get_pearson_correlation(df, df_info)
        with warnings.catch_warnings(record=True) as w:
            result = get_correlation("target", "not_a_col", df_info)
            assert result is None
